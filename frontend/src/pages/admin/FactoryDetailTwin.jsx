  ...
  return <div className="twin-theme-page factory-detail-twin min-h-full bg-[#050b12] px-5 py-5 text-slate-100 md:px-7 md:py-6">
    <header className="twin-theme-hero rounded-3xl border border-cyan-400/15 bg-[radial-gradient(circle_at_85%_10%,rgba(0,210,255,.12),transparent_30%),linear-gradient(135deg,#0b1a27,#071019)] p-6 shadow-2xl">
      <div className="flex flex-col justify-between gap-4 xl:flex-row xl:items-center">
        <div className="flex items-start gap-4"><button onClick={onBack} className="mt-1 grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-white/10 bg-white/[.035] text-slate-300 hover:text-cyan-200"><ArrowLeft size={17}/></button><div><div className="flex items-center gap-2 text-[9px] font-black tracking-[.2em] text-cyan-300"><Factory size={14}/> HANWHA OCEAN · FACILITY SAFETY TWIN</div><h1 className="mt-2 text-2xl font-black text-white">{factory.name}</h1><p className="mt-1 text-xs text-slate-400">{factory.code} · {factory.profileLabel} · 작업자·허가·설비·센서 통합 운영</p><p className="mt-1 text-[10px] text-slate-600">{isWeldingDemo ? "보유 로봇·용접 품질 CSV를 공통 입력 스키마로 재생하는 PoC 구성입니다" : "설비 프로필 기반 검증 데이터이며 센서·게이트웨이 연결 시 동일 스키마로 교체할 수 있습니다"}</p></div></div>
        <div className="flex flex-wrap items-center gap-2"><span className="sim-badge">VALIDATION DATA</span><span className={`flex items-center gap-2 rounded-xl border px-3 py-2 text-[9px] font-black ${isWeldingDemo ? "border-emerald-400/20 bg-emerald-400/[.06] text-emerald-300" : "border-amber-400/20 bg-amber-400/[.06] text-amber-200"}`}><Radio size={13}/> {isWeldingDemo ? "CSV DATASET LOADED" : "향후 데이터 연동 대상"}</span><span className="rounded-xl border border-white/10 bg-white/[.035] px-3 py-2 text-[9px] font-bold text-slate-400">{isWeldingDemo ? `SAMPLE · T+${weldingDemo.telemetry.t_sec.toFixed(0)}s` : "CONCEPT LAYOUT · DATA ADAPTER READY"}</span></div>
      </div>
    </header>
    ...
    <section className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
      ...
      <aside className="twin-theme-panel rounded-2xl border border-cyan-400/15 bg-[#08131d] p-4 shadow-xl">
        <p className="text-[9px] font-black tracking-[.16em] text-cyan-300">OPERATION INVENTORY <span className="text-slate-600">(VALIDATION DATA)</span></p>
  ...
  return <section className="mt-4 space-y-4">
    ...
    <div className="grid gap-4 xl:grid-cols-[1.08fr_.92fr]">
      <article className="rounded-2xl border border-cyan-400/15 bg-[#08131d] p-5">
        <div className="flex items-start justify-between gap-3"><div><p className="text-[9px] font-black tracking-[.16em] text-cyan-300">ROBOT CONDITION · ASS-04-01</p><h3 className="mt-2 text-sm font-black text-white">CSV 기반 로봇 상태</h3></div><span className={`rounded-full border px-2.5 py-1 text-[8px] font-black ${statusTone}`}>{demo.status}</span></div>
      ...
      <article className="rounded-2xl border border-cyan-400/15 bg-[#08131d] p-5">
        ...
        <p className="mt-3 text-[8px] leading-4 text-slate-600">이 화면의 숫자는 두 CSV의 현재 행과 모델 출력값을 재생합니다. 현장 센서 데이터도 동일 입력 스키마로 연결해 재검증할 수 있습니다.</p>
      </article>
    </div>
  </section>;
}
...
function WorkerSafetyPanel({ worker, factory, responseState, notify }) {
  ...
  return <article className={`rounded-2xl border p-5 ${worker.riskLevel === "HIGH" ? "border-red-400/25 bg-red-500/[.045]" : "border-emerald-400/15 bg-[#08131d]"}`}>
    ...
    <div className="mt-3 rounded-xl border border-white/[.07] bg-black/10 p-4"><p className="text-[9px] font-black text-slate-500">PPE CHECK</p><div className="mt-3 grid grid-cols-3 gap-2"><Ppe label="안전모" ok={worker.ppe.helmet}/><Ppe label="안전대" ok={worker.ppe.harness}/><Ppe label="안전조끼" ok={worker.ppe.vest}/></div></div>
...
function WorkerPermitPanel({ worker }) { return <article className="rounded-2xl border border-cyan-400/10 bg-[#08131d] p-5"><p className="text-[9px] font-black tracking-[.16em] text-cyan-300">PERMIT TO WORK LINK</p><h2 className="mt-2 text-sm font-black text-white">작업허가서 검증</h2><div className="mt-4 rounded-xl border border-white/[.07] bg-white/[.025] p-4"><div className="flex items-center justify-between"><span className="font-mono text-[10px] text-cyan-200">{worker.permitId}</span><span className={`rounded-full px-2 py-1 text-[8px] font-black ${worker.permitStatus === "VALID" ? "bg-emerald-400/10 text-emerald-300" : "bg-amber-400/10 text-amber-300"}`}>{worker.permitStatus}</span></div><div className="mt-4 space-y-3"><Row label="허가 유형" value={worker.permitType}/><Row label="작업 위치" value={worker.assignedAssetName}/><Row label="승인 작업" value={worker.task}/><Row label="유효 시간" value={`오늘 ${worker.permitExpiresAt}까지`}/></div></div><div className="mt-3 rounded-xl border border-emerald-400/15 bg-emerald-400/[.04] p-4"><div className="flex gap-3"><FileCheck2 size={17} className="shrink-0 text-emerald-300"/><div><b className="text-[10px] text-emerald-200">작업·위치·담당 설비 일치</b><p className="mt-1 text-[9px] leading-4 text-slate-500">현재 검증 데이터에서 허가 범위 이탈과 만료 여부를 확인합니다. 허가·위치 데이터를 교체해도 동일 규칙으로 검증할 수 있습니다.</p></div></div></div><div className="mt-4 flex items-center gap-2 text-[8px] text-slate-600"><Link2 size={12} className="text-cyan-300"/> 작업허가서 · 작업자 위치 · 설비 태그 연결</div></article>; }
