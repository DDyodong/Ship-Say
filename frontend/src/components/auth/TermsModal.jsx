...
function TermsModal({ open, onClose, section = "all" }) {
  useEffect(() => {
  ...
  const showTerms = section === "all" || section === "terms";
  const showPrivacy = section === "all" || section === "privacy";
  const title = section === "privacy" ? "개인정보 처리방침" : section === "terms" ? "서비스 이용약관" : "이용약관 및 개인정보 수집·이용";
  ...
  return <div className="terms-modal-backdrop" role="presentation" onMouseDown={onClose}>
    <section className="terms-modal" role="dialog" aria-modal="true" aria-labelledby="terms-modal-title" onMouseDown={event=>event.stopPropagation()}>
      <header>
        <div><span>SMART SHIPYARD</span><h3 id="terms-modal-title">{title}</h3></div>
        <button type="button" onClick={onClose} aria-label="약관 닫기"><X/></button>
      </header>
      <div className="terms-modal-content">
        {showTerms && <><h4>서비스 이용약관</h4>
          ...
          <p>서비스에서 제공되는 위험 분석과 안전 정보는 현장 안전관리 업무를 보조하기 위한 자료이며, 사용자는 회사의 안전 절차와 관리자의 지시를 우선하여 준수해야 합니다.</p></>}
        {showPrivacy && <><h4>개인정보 수집·이용</h4>
          <dl>
