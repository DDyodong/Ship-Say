    ...
    <div ref={containerRef} className="relative h-full">
      ...
      <div className="absolute left-4 z-10 w-52 rounded-2xl border border-cyan/15 bg-ink/55 backdrop-blur-xl shadow-xl overflow-hidden" style={{ top: filterPanelTop }}>
        <button onClick={() => setPanelCollapsed((v) => !v)}
          className="w-full flex items-center justify-between gap-1.5 px-3 py-2.5 text-[12px] font-bold text-slate-200 hover:bg-white/5 transition-colors">
        ...
        {!panelCollapsed && <div className="px-3 pb-3 flex flex-col gap-3">
          <div>
            <div className="flex flex-col gap-1">
              {typesPresent.map((value) => <button key={value}
                ...
                className={`text-left px-2 py-1.5 rounded-md text-[12px] font-mono transition-all ${filter === value
          ...
          <div>
            <div className="text-[12px] font-bold text-slate-200 mb-1.5">지도 조작</div>
          ...
          <div>
            <div className="text-[12px] font-bold text-slate-200 mb-1.5">시설 현황</div>
      ...
      <div className="absolute z-10 flex flex-col gap-2" style={{ right: overlayPanel ? 340 : 16, top: topOffset }}>
        <div className={`px-2.5 py-1.5 backdrop-blur-md rounded text-[12px] font-bold flex items-center gap-1.5 ${
          ...
          <i className={`fa-solid fa-camera text-[12px] ${activeCameraAlerts.length ? "text-white" : cameraConnected ? "text-cyan" : "text-slate-500"}`}/>
      ...
      <div className="absolute left-4 z-10 px-3 py-1.5 rounded-lg bg-ink/55 backdrop-blur-md border border-white/10 text-[12px] text-slate-300" style={{ bottom: bottomOffset }}>
      ...
      {selected && <div className="absolute left-1/2 -translate-x-1/2 z-10 w-[min(90%,520px)] rounded-xl bg-ink/70 backdrop-blur-xl border border-white/10 px-4 py-3 flex items-center gap-4" style={{ bottom: bottomOffset + 60 }}>
        ...
        <div className="min-w-0">
          ...
          <p className="text-[12px] font-mono text-slate-400">{typeLabels[selected.type]} · {selected.lat.toFixed(5)}, {selected.lng.toFixed(5)}</p>
          {cameraAlertByFacility.has(selected.code) && <p className="text-[12px] font-bold text-danger mt-1">
