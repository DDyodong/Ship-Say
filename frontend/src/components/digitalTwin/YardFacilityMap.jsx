...
function YardFacilityMap({
  ...
  const openSelected = () => {
    if (!selected) return;
    if (selected.code === "T-BAR-SHOP") {
      onOpenShop?.(selected.code);
      return;
    }
    onUnavailable?.(selected.name);
  };
      ...
      <div className="facility-map-shell">
        ...
        <div className="facility-marker-layer" aria-label="조감도 시설 위치">
          ...
          {workers.map((worker) => (
            <div
              key={worker.id}
              className={`facility-worker${worker.danger ? " danger" : ""}`}
              style={{
            ...
            >
              <i />
              <span>{worker.label || `ID:${worker.id}`}</span>
            </div>
          ))}
        </div>
...
const FACILITY_MAP_CSS = `
  .facility-map-card{overflow:hidden;border:1px solid rgba(60,154,180,.28);border-radius:18px;background:#06131c;box-shadow:0 24px 70px rgba(0,0,0,.3)}
  ...
  .facility-map-background{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:center;user-select:none}
  ...
  .facility-external-panel{position:absolute;z-index:25;right:24px;top:72px;width:350px;display:flex;flex-direction:column;gap:10px}
  ...
  @keyframes facilityPulse{50%{box-shadow:0 0 0 12px rgba(255,70,100,.14),0 5px 18px rgba(0,0,0,.52)}}@keyframes workerPulse{50%{transform:scale(1.45);opacity:.55}}@keyframes alertBlink{50%{opacity:.35}}
